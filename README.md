...
| **Apache HTTP Server** | Apache/2.4.52 (Ubuntu/Debian) |
| **TShark version** | TShark 4.2.2 |
| **Wireshark version** | Wireshark 4.2.2 |
| **curl version** | curl 7.88.1 |
| **Server IP** | 192.168.199.128 |
| **Client/source IP** | 192.168.199.128 |
| **Interface** | lo / eth0 |
| **TCP service port** | 80 |
| **Capture filename** | `_baseline_apache.pcapng` |

---

## 6. Establish the Normal Apache/TCP Baseline
Started the authorised Apache service and verified that TCP/80 is listening. Established an ordinary connection from the laboratory client and captured the complete TCP three-way handshake.
...
curl -I [http://192.168.199.128/](http://192.168.199.128/)
...
### Baseline Handshake Verification
...
| **3** | ACK | 3 | 192.168.199.130:48210 | 192.168.199.128:80 | Seq=1, ACK=1 | 0.000588 |

> **Figure 1 Reference:** Terminal output confirms Apache HTTP server is active and running on 192.168.199.128 on Tue Sep 08 05:08:12 EDT 2026.
...
| --- | --- |
| **Target** | 192.168.199.135 |
| **Target port** | 80 |
| **Authorised packet count** | 4 |
| **Source/loopback interface** | eth0 / lo |
| **Start time** | 09:50:10 EDT |
| **End time** | 09:50:14 EDT |
| **Capture filename** | `bounded_simulation.pcapng` |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| **1** | 18 | Sep 8, 2026 06:16:34.666237405 EDT | 192.168.199.135 | 80 | 45306 | 1 | 0 |
...
| **Unique source ports** | 1 | *(Run on attack PCAP)* | Single source port (45306) used |
| **Destination port** | 80 | 80 | Web server target port |
| **Capture duration** | *(See stats)* | *(See stats)* | Baseline traffic collected in seconds |
...
| --- | --- | --- | --- | --- |
| **0.000000** | 1 | Initial SYN | 127.0.0.1:48030 → 127.0.0.1:80 | Connection requested. |
| **0.000031** | 2 | SYN-ACK | 127.0.0.1:80 → 127.0.0.1:48030 | Server acknowledges request and allocates socket resources. |
| **0.000052** | 3 | ACK | 127.0.0.1:48030 → 127.0.0.1:80 | Handshake completed successfully; legitimate session established. |
| **0.012410** | 11 | Initial SYN | 192.168.199.135:55922 → 151.101.129.91:80 | Outbound request targeting external HTTP web server. |
| **0.012550** | 13 | SYN-ACK | 151.101.129.91:80 → 192.168.199.135:55922 | External web server responds, acknowledging connection request. |
| **0.012610** | 14 | ACK | 192.168.199.135:55922 → 151.101.129.91:80 | Handshake finalized; ready for HTTP GET request transmission. |
...
| **Handshake completion** | Expected | Failed / Incomplete | Typical signature of a SYN flood attack pattern (hping3 mechanism). |
| **Repeated initial SYNs** | Limited | High / Rapid succession | High-frequency burst of SYN packets originating from automated tool activity. |
| **Unique source ports** | Varies | Rapid incrementation | Multiple distinct source ports opened in rapid succession to target port 80. |
| **RST behaviour** | Possible | None / Minimal | No active connection reset issued by client or host during burst window. |
| **Server resource impact** | Low | Potential backlog exhaustion | Half-open states force server to retain resources in SYN queue until timeout. |
...
1. **Rate Monitoring:** Monitor TCP/80 for abnormal increases in initial SYN rate compared with a normal baseline.
2. **Flag Ratio Alerts:** Track SYN-to-SYN-ACK and SYN-to-final-ACK ratios over time. Alert on sustained high incomplete-handshake counts rather than a single SYN.
3. **Port Tracking:** Record source IP and source-port diversity, while avoiding treating diversity alone as proof of malicious activity.
4. **Log Correlation:** Correlate packet captures with Apache access/error logs and host resource metrics.
5. **SYN Backlog Controls:** Use appropriate SYN backlog protection/SYN cookies (`tcp_syncookies`) where supported and approved.
6. **Firewall & Upstream:** Apply firewall/rate-limiting controls at the authorised network boundary or upstream DDoS protection.
7. **Time Sync:** Maintain accurate time synchronisation (NTP) so packet and server timelines can be correlated.
8. **Evidence Preservation:** Preserve captures and logs with SHA-256 hashes for forensic review.
...
1. **Capture Window Scope:** Offline PCAP captures cover limited time windows and may not reflect complete connection lifecycles.
2. **Packet Loss Ambiguity:** Network drops or capture point restrictions can simulate incomplete handshakes even when completed on the wire.
3. **IP Spoofing Constraints:** Header IP addresses verify packet origin claims but do not establish physical device or human attribution.
4. **Simulation Scale:** Bounded laboratory traffic does not duplicate real-world distributed denial-of-service (DDoS) volume.
...
While packet captures conclusively prove the presence of rapid SYN requests lacking final ACK completions, packet evidence alone is insufficient to assert operational service denial or system resource depletion. Definitive attribution and impact assessments require multi-source correlation incorporating web server logs, host socket metrics, and infrastructure security logs. All primary evidence files have been cryptographically hashed and preserved to maintain chain-of-custody integrity.
